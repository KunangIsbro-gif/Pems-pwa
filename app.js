...
async function registerServiceWorker() {
  ...
  try {
    await navigator.serviceWorker.register('./service-worker.js?v=v15-9-27-r13f-hf26d');
  } catch (err) {
...
async function bootAuthenticated() {
  try {
    ...
    if (navigator.onLine) {
      ...
      // HF26D: /bootstrap may temporarily return an empty PROJECTS cache
      ...
      if (!Array.isArray(boot?.projects) || boot.projects.length === 0) {
        try {
          ...
          console.warn('HF26D project bootstrap recovery:', projectErr);
        }
      } else {
        state.projectLoadError = '';
      }
...
// HF26D - manual reload is a safe, non-destructive GET, keeping role filters.
async function refreshProjectsHF26DPEMS_() {
  if (!navigator.onLine) {
    toast('Periksa koneksi internet dahulu.', 'warning', 5000);
    return;
  }
  const btn = document.getElementById('refreshProjectsHF26DBtn');
  try {
    ...
    const latestBtn = document.getElementById('refreshProjectsHF26DBtn');
...
async function renderHome() {
  ...
  document.getElementById('refreshProjectsHF26DBtn')?.addEventListener('click', refreshProjectsHF26DPEMS_);
...
async function renderWorkCoreHF17PEMS_(renderSeq) {
  const projects = state.bootstrap?.projects || [];
  if (!projects.length) {
    el.content.innerHTML = `<div class="status-box warning"><b>Project belum dapat dimuat.</b> ${escapeHtml(state.projectLoadError || 'Periksa koneksi atau assignment akun ini.')} <button id="refreshProjectsHF26DBtn" type="button" class="btn secondary small" ${navigator.onLine ? '' : 'disabled'}>Muat Ulang Project</button></div>`;
    document.getElementById('refreshProjectsHF26DBtn')?.addEventListener('click', refreshProjectsHF26DPEMS_);
    return;
  }
...
async function renderAdmin() {
  ...
  try {
    ...
    // HF26D: never silently erase the Home project list when the Admin endpoint
